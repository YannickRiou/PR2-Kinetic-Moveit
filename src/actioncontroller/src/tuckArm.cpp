...
void createActionControllerMessage(std::string group_id, float x, float y, float z, float w, actioncontroller::ActionControllerGoal &goal){	
...
void callingActionController(std::string part, actioncontroller::ActionControllerGoal &goal){
	...
	ROS_INFO("Sending goal for  ActionController");
	ac_.sendGoal(goal);
...
int main(int argc, char** argv)
{
	...
	std::string part = "left_arm";
	createActionControllerMessage(part, 0.28, 0.2, 0.5, 1.0, goal);
	...
	part = "base";
	createActionControllerMessage(part, 1, 0, 0, 1.0, goal);
	callingActionController(part, goal);
