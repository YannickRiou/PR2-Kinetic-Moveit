...
namespace visp_auto_tracker{
        ...
        void Node::frameCallback(const sensor_msgs::ImageConstPtr& image, const sensor_msgs::CameraInfoConstPtr& cam_info){
                ...
                ROS_INFO("Image recieved by tracker");
                got_image_ = true;  
        }
        ...
        void Node::spin(){
                ...
                if (cmd_.get_detector_type() == CmdLine::ZBAR){
                        ROS_INFO("trying to detect a QRcode");
                        detector = new vpDetectorQRCode;
                }
                else if(cmd_.get_detector_type() == CmdLine::DMTX){
                    ROS_INFO("trying to detect a matrix");
                    detector = new vpDetectorDataMatrixCode;
                }
                else{
                    ROS_INFO("Invalid Marker Type");
                }
#endif 
#if 0
                ...
                while(ros::ok()){
                        ...
                        ROS_INFO("trying to locate a Marker");
                        ...
                        //std::string s;
                        //s = std::to_string( object_pose_publisher.getNumSubscribers() ) ;
                        //ROS_INFO( "%s" , s.c_str() );
